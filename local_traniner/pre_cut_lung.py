...
def get_segmented_lungs(img_dir, plot=False, min_lung_area=3000, padding_num=0):
    ...
    if mask_img.mean() < 7:
        ...
        filled_crop_img = np.array(filled_crop_img, dtype=int)
...
#             mn = np.min(image)
#             if mx - mn > 0:
#                 image = np.array((image - mn) / (mx - mn) * 255, dtype=int16)
#                 crop_img = np.array((crop_img - mn) / (mx - mn) * 255, dtype=int16)
#                 filled_img = np.array((filled_img - mn) / (mx - mn) * 255, dtype=int16)
